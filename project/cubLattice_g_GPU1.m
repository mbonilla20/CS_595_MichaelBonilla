function [q,out_param,y,kappanumap] = cubLattice_g_GPU1(varargin)
...
xpts=mod(bsxfun(@plus, lattice_gen_GPU(1,n0,out_param.d), out_param.shift),1); %grab Lattice points
...
for m=out_param.mmin+1:out_param.mmax
   ...
   mnext=m-1;
   nnext=2^mnext;
   xnext=mod(bsxfun(@plus, lattice_gen_GPU(nnext+1,2*nnext,out_param.d), out_param.shift),1);
   n0=n0+nnext;
   ynext=gather(f(xnext));
